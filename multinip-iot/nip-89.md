...
```jsonc
{
  ...
  "tags": [
    ["d", <random-id>],
    ["k", <supported-event-kind>, <optional-templates...>],
...
* `k` tags' first value is the event kind that is supported by this `kind:31990`
...
### Specify event templates in k tag

In the k tag it is possible to specify the template for the content and tags of the supported event kind
```jsonc
    ["k", <supported-event-kind>, <event-template>, <input-fields-schema>]
```

* `supported-event-kind` is [{{mustache}}](https://mustache.github.io/) template detailing the layout of the content and tags fields of the supported event
    * eg. 
        ```mustache
        {
            "content": "Hello {{name}}",
        ...
        ```
* `input-fields-schema` is a [JSON Schema](https://json-schema.org/) detailing the expected input fields for the supported event kind
    * eg.
        ```json
        {
            "type": "object",
